...
clean_exit() {
	local exit_code="$1"
	echo "entering clean_exit procedure with $exit_code"
	...
	echo "leaving clean_exit procedure with $exit_code, all done !"
...
main() {
	local config=
	# LRO : do not pass user name, will be dynamic
	...
	# do not pass user, will be dynamic
	#if [[ -z "${user}" ]]; then
	...
	nsdo openvpn \
		--config "${config}" \
		--auth-user-pass /root/.protonvpn-cli/protonvpn_openvpn_credentials \
		--auth-retry nointeract \
		--verb 4 \
		--log /root/.protonvpn-cli/connection_log_manual &
	...
	>&2 printf "openvpn is active (pid = %d), removing default route\n" "${openvpn_pid}"
	...
	${localdir}/vpnshift-server-open.sh $namespace $config
}
...
exec > >(tee -i ${logdir}/vpnshift.log)
exec 2>&1

main "$@"
echo "exit of vpnshift completed OK"
